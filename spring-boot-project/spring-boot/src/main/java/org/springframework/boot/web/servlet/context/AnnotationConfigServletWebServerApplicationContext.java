...
public class AnnotationConfigServletWebServerApplicationContext extends ServletWebServerApplicationContext
	...
	public AnnotationConfigServletWebServerApplicationContext() {
		/**
		 * TODO 创建reader，并添加注解支撑，包括注解支持（AnnotatedBeanDefinitionReader是一个读取注解的Bean读取器），可以看spring源码中的去除注解案例
		 *
		 * 这里将spring的主要注解支撑注册到beanDefinition注册表中
		 * 	1、ConfigurationClassPostProcessor类型的注解，比如@Configuration @Bean @Component、@ComponentScan、@Import、@ImportResource
		 * 	2、AutowiredAnnotationBeanPostProcessor类型的注解，比如@Autowired、@Value、@Inject
		 */
		this.reader = new AnnotatedBeanDefinitionReader(this);
		//扫描的包（作用就是将指定包下的类通过一定规则过滤后 将Class 信息包装成 BeanDefinition 的形式注册到IOC容器中）
